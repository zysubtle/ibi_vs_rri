...
static void ppg_ibi_reset_detector(ppg_ibi_context_t *ctx) {
  ctx->has_prev_sample = 0u;
  ctx->has_prev2_sample = 0u;
  ctx->prev_raw = 0;
  ctx->prev2_raw = 0;
  ctx->prev_selected_channel = PPG_IBI_SELECTED_CHANNEL_INVALID;
  ctx->prev2_selected_channel = PPG_IBI_SELECTED_CHANNEL_INVALID;
  ctx->prev_signal_quality = 0u;
  ctx->prev2_signal_quality = 0u;
...
static uint8_t ppg_ibi_is_ppg_saturated(const ppg_ibi_sample_t *sample) {
  for (size_t ch = 0; ch < (size_t)PPG_IBI_CHANNEL_COUNT; ++ch)
    if (ppg_ibi_channel_quality(sample->ppg[ch]) ==
        PPG_IBI_CHANNEL_QUALITY_INVALID)
      return 1u;
  return 0u;
}
static void ppg_ibi_select_channel(const ppg_ibi_sample_t *sample,
                                   ppg_ibi_event_t *event) {
  uint8_t bq = PPG_IBI_CHANNEL_QUALITY_INVALID,
          bc = PPG_IBI_SELECTED_CHANNEL_INVALID;
  for (size_t ch = 0; ch < (size_t)PPG_IBI_CHANNEL_COUNT; ++ch) {
    uint8_t q = ppg_ibi_channel_quality(sample->ppg[ch]);
    if (q > bq) {
      bq = q;
      bc = (uint8_t)ch;
    }
  }
  event->signal_quality = bq;
  event->selected_channel = bc;
  if (bc != PPG_IBI_SELECTED_CHANNEL_INVALID)
...
ppg_ibi_status_t ppg_ibi_init(ppg_ibi_context_t *ctx,
  ...
  ppg_ibi_config_t local;
  if (!ctx)
    return PPG_IBI_STATUS_INVALID_ARGUMENT;
  memset(ctx, 0, sizeof(*ctx));
  ppg_ibi_config_default(&local);
  ctx->config = config ? *config : local;
...
ppg_ibi_status_t ppg_ibi_process(ppg_ibi_context_t *ctx,
  ...
  if (sample->allow_measure == 0u) {
    ...
    if (ppg_ibi_is_ppg_saturated(sample)) {
      event->reject_reason = PPG_IBI_REJECT_SATURATED;
      event->debug_flags |= PPG_IBI_DEBUG_FLAG_PPG_SATURATED;
      strict_reject = 1u;
    }
    if ((ctx->has_last_timestamp != 0u) &&
        (ctx->config.allow_timestamp_strict_check != 0u)) {
      uint32_t dt = sample->timestamp_ms - ctx->last_timestamp_ms;
      if (dt > (uint32_t)ctx->config.expected_interval_ms) {
        ...
        strict_reject = 1u;
      } else if (dt != (uint32_t)ctx->config.expected_interval_ms) {
        if (event->reject_reason == PPG_IBI_REJECT_NONE)
    ...
    if (strict_reject != 0u) {
      ...
      if ((ctx->has_prev_sample != 0u) && (ctx->has_prev2_sample != 0u) &&
        ...
        if (ctx->has_last_pulse == 0u) {
          ...
          if ((ibi >= (uint32_t)ctx->config.min_ibi_ms) &&
              (ibi <= (uint32_t)ctx->config.max_ibi_ms)) {
            ctx->beat_count += 1u;
            event->timestamp_ms = ctx->prev_timestamp_ms;
            event->sample_index = ctx->prev_sample_index;
            event->selected_channel = ctx->prev_selected_channel;
            event->signal_quality = ctx->prev_signal_quality;
            event->confidence = ctx->prev_signal_quality;
...
const char *ppg_ibi_version(void) { return "0.6.0-m6-fix7"; }
