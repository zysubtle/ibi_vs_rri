...
static void ppg_ibi_fill_no_event(
...
static void ppg_ibi_select_channel(const ppg_ibi_sample_t *sample, ppg_ibi_event_t *event) {
    ...
    for (ch = 0u; ch < (size_t)PPG_IBI_CHANNEL_COUNT; ++ch) {
        const uint8_t quality = ppg_ibi_channel_quality(sample->ppg[ch]);
...
ppg_ibi_status_t ppg_ibi_init(ppg_ibi_context_t *ctx, const ppg_ibi_config_t *config) {
    ...
    ctx->config = (config != NULL) ? *config : local;
...
ppg_ibi_status_t ppg_ibi_process(
    ppg_ibi_context_t *ctx,
    const ppg_ibi_sample_t *sample,
    ppg_ibi_event_t *event) {
    ...
    if (sample->allow_measure == 0u) {
        ...
        if ((ctx->has_last_timestamp != 0u) && (ctx->config.allow_timestamp_strict_check != 0u)) {
            const uint32_t dt = sample->timestamp_ms - ctx->last_timestamp_ms;
        ...
        if (strict_reject != 0u) {
            ...
            const int32_t raw = sample->ppg[event->selected_channel];
            ...
            if ((ctx->has_prev_sample != 0u) &&
                ...
                if (ctx->has_last_pulse == 0u) {
                    ...
                    const uint32_t ibi = ctx->prev_timestamp_ms - ctx->last_pulse_timestamp_ms;
