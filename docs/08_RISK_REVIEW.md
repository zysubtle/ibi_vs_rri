...
- 当前 detector 为 synthetic-first 的三点局部峰策略，无滤波、无去趋势、无模板约束，真实数据误检/漏检风险高（S2）。
- 无 gold standard 与人工标注，M5 仅验证工程闭环，不能用于准确性宣称（S2）。
- 异常段会清理 detector history 以避免跨段伪 IBI，但恢复期可能增加漏检，需在 M6/M7 复盘（S2）。
