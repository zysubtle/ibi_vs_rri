...
- `IBI_OUT_OF_RANGE` 后强制转入 `REACQUIRE`，降低在 `TRACK` 中持续误跟踪导致的连续假拒绝风险。
- `EVENT_READY` 返回前补齐 detector history 连续性，降低下一拍重复命中同一 candidate pulse 并触发 `IBI_OUT_OF_RANGE` 的风险。
