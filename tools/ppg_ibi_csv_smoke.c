...
int main(void) {
  ...
  fprintf(events,
          "timestamp_ms,sample_index,ibi_ms,beat_count,confidence,signal_"
          "quality,selected_channel,state,reject_reason,debug_flags\n");
  ...
  while (fgets(line, sizeof(line), in) != NULL) {
    ...
    if (event.reject_reason == PPG_IBI_REJECT_ALLOW_MEASURE_FALSE)
      reject_allow++;
    if (event.reject_reason == PPG_IBI_REJECT_LOW_SIGNAL_QUALITY)
      reject_low++;
    if (event.reject_reason == PPG_IBI_REJECT_SATURATED)
      reject_sat++;
    if (event.reject_reason == PPG_IBI_REJECT_TIMESTAMP_GAP)
      reject_gap++;
    if (event.reject_reason == PPG_IBI_REJECT_SAMPLE_DROP)
      reject_drop++;
    if (event.reject_reason == PPG_IBI_REJECT_IBI_OUT_OF_RANGE)
      reject_ibi++;

    if (status == PPG_IBI_STATUS_EVENT_READY) {
      uint32_t ibi = (uint32_t)event.ibi_ms;
      event_ready_count++;
      if (event_ready_count == 1U || ibi < ibi_min)
        ibi_min = ibi;
      if (event_ready_count == 1U || ibi > ibi_max)
        ibi_max = ibi;
      if (ibi < PPG_IBI_MIN_IBI_MS || ibi > PPG_IBI_MAX_IBI_MS)
        ibi_oor++;

      fprintf(events, "%u,%u,%u,%u,%u,%u,%u,%d,%d,%u\n", event.timestamp_ms,
              event.sample_index, event.ibi_ms, event.beat_count,
              event.confidence, event.signal_quality, event.selected_channel,
              (int)event.state, (int)event.reject_reason, event.debug_flags);
    }
  }
